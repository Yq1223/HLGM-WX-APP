...
Page({
  data: {
    ...
    importResult: { successCount: 0, failCount: 0, failDetails: [] }
  },

  onLoad(options) {
    if (options.mode === 'edit') {
      // 编辑模式 - 从 globalData 读取数据
  ...
  onCategory(e) {
    ...
    this.setData({ 'form.category': this.data.form.category === val ? '' : val });
  },
  ...
  doImport(filePath) {
    ...
    uploadFile({
      ...
      this.setData({
        ...
        importResult: res.data || { successCount: 0, failCount: 0, failDetails: [] }
      });
    }).catch(err => {
      wx.hideLoading();
  ...
  closeImportResult() {
    this.setData({ showImportResult: false });
  },
  ...
  onSubmit() {
    ...
    ensureLogin().then(() => {
      ...
      const apiRequest = isEdit
        ? request({ url: '/api/wool/update/' + editId, method: 'PUT', data })
        : request({ url: '/api/wool/publish', method: 'POST', data });

      apiRequest.then(() => {
        wx.showToast({
          title: isEdit ? '修改成功，等待审核' : '发布成功，等待审核',
          icon: 'success',
          duration: 2000
        });
        setTimeout(() => {
          if (isEdit) {
            ...
            this.setData({
              form: { title: '', content: '', category: '', sourceUrl: '', claimSteps: '' }
            });
          }
        }, 1500);
      }).catch(() => {}).finally(() => {
